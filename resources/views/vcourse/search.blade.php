...
$(document).ready(function(){
    ...
    $(".search_condition li").click(function(){//点击筛选分类
        ...
        var url = '{{route('vcourse.search')}}';

        url = url+'?search_key='+searchkey;

        if (cname=='agency_id') {
            url = url+'&agency_id='+value+'&type={{request('type')}}';
        } else if(cname=='type') {
            url = url+'&type='+value+'&agency_id={{request('agency_id')}}';
        } else {
            url = url+'&type={{request('type')}}&agency_id={{request('agency_id')}}';
        }
        location.href = url;
    });
